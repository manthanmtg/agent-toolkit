...
export async function getDeployedSkillsPerTool(): Promise<DeployedSkillsResult> {
  ...
  for (const toolId of toolsToScan) {
    ...
    const expectedMap = buildExpectedHashes(allSkills, toolId, profile, patterns[0].dir);
    ...
    // Build expected hashes using claude-code adapter for cross-agent skills
    // since the files are in Claude Code's format
    const crossExpected = buildExpectedHashes(allSkills, "claude-code", profile, "skills");

    for (const cross of crossPaths) {
    ...
    skills.sort((a, b) => {
      const order: Record<DeployedSkillStatus, number> = { outdated: 0, unknown: 1, "up-to-date": 2 };
