...
export function SkillsList({ skills }: { skills: Skill[] }) {
  ...
  const domains = [...new Set(filtered.map((s) => s.domain))].sort();

  const localCount = skills.filter((s) => s.source === "local").length;
  const toolkitCount = skills.filter((s) => s.source === "toolkit").length;
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            ...
            {[...new Set(skills.map((s) => s.domain))].length} domains
          </p>
        </div>
        <Link
          href="/skills/new"
