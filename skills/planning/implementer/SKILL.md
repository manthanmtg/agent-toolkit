...
description: >
  ...
  implementation checklist. Use when the user asks to execute a plan, implement
  planned work, continue from a design doc, or build according to documented
  requirements and questions are allowed when necessary.
domain: planning
version: 1.0.0
tags: [implementation, execution, plan, engineering, verification]
...
Execute an approved plan with disciplined engineering practices. Preserve the
plan's intent, but verify details against the live codebase before editing.
...
- Ask the user concise questions when the plan is incomplete, contradictory, or
  would require choosing between materially different behaviors.
- Do not invent broad requirements beyond the plan.
...
- Check whether the plan is current against the repository. If the code has
  drifted, update the execution approach and note the drift.
- Ask the user before proceeding if a missing or stale plan detail changes the
  intended behavior.
